...
if [ $COUNTERFACTUAL_COUNT -lt $(($LABEL_COUNT + 4)) ]; then
    echo "Out of $COUNTERFACTUAL_COUNT counterfactuals $LABEL_COUNT have been labeled; generating more"
    python experiments/generate_counterfactual.py --result_dir $RESULT_DIR --strategy random-nearest --count 4 --counterfactual_max_iters 100
else
